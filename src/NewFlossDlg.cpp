...
void NewFlossDlg::on_DialogButtonBox_accepted()
{
    if (!m_flossScheme->find(ui.FlossName->text()) ||
        KMessageBox::questionTwoActions(this, i18n("The floss name %1 is already used.\nOverwrite with the description and color selected.", ui.FlossName->text()), i18n("Overwrite"), KStandardGuiItem::overwrite(), KGuiItem(i18nc("@action:button", "Do Not Overwrite"), QStringLiteral("dialog-cancel"))) == KMessageBox::PrimaryAction) {
